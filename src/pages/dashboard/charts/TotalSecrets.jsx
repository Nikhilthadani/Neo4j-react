...
const TotalSecretsChart = () => {
  ...
  const { averageRiskScore } = useMemo(() => {
    const totalVerified = repositories.reduce(
      (sum, repo) => sum + (repo.verifiedFindings || 0),
      0
    );
    const totalUnverified = repositories.reduce(
      (sum, repo) => sum + (repo.unverifiedFindings || 0),
      0
    );
  ...
  const series = useMemo(
    () => [Math.round(averageRiskScore * 100)],
    [averageRiskScore]
  );
